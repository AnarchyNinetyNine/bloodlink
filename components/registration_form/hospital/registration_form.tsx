...
import NameBlock from '@/components/registration_form/hospital/name_block';
...
const RegistrationForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/auth/hospital/sign-up', {
        method: 'POST',
        headers: {
      ...
      if (response.status === 201) {
        ...
        setTimeout(() => (window.location.href = '/org/account/sign-in'), 3000);
      } else {
        toast.dismiss(toastId);
  ...
  return (
    <div className="w-96 mx-auto">
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        <NameBlock hospitalName={hospitalName} setHospitalName={setHospitalName} />
